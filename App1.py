...
def find_closest_precaution_medicine(predicted_class, temp, humidity, ph, light, rain):
    ...
    # Check if class_df is empty
    if class_df.empty:
        print(f"[DEBUG] No matching data found for predicted class: {predicted_class}")
        return {
            "Precautions": "No data available for this disease.",
            "Medicines": "No data available for this disease."
        }
    ...
    # Extra safety check after scoring (optional)
    if class_df.empty or class_df["distance_score"].isnull().all():
        print(f"[DEBUG] No usable rows for class: {predicted_class}")
        return {
            "Precautions": "No usable data found for this disease.",
            "Medicines": "No usable data found for this disease."
        }
